...
**CRITICAL**: Challenges MUST be generated by your server, never on the client-side. This library enforces this security requirement:

- ❌ **Client-side challenge generation has been removed** for security
- ✅ **Server-generated challenges are required** for all operations
- ✅ **Use `registerRemote()` and `authenticateRemote()`** for automatic server integration
- ✅ **Or provide server-generated challenges** directly in your configuration

Challenges are designed to prevent replay attacks and are a fundamental security component of WebAuthn. Generating them on the client would defeat their purpose.

```typescript
// ✅ SECURE: Use remote methods (recommended)
this.webAuthnService.registerRemote({ username: 'user@example.com' });
this.webAuthnService.authenticateRemote();

// ✅ SECURE: Provide server-generated challenge
this.webAuthnService.register({
  username: 'user@example.com',
  preset: 'passkey',
  challenge: challengeFromYourServer // Must come from server
});
```
